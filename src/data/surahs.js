// Static surah metadata (Hafs/Uthmani numbering): number, English name, ayah count.
// Lets lightweight screens (Home) resolve a surah name or a goal's verse keys without
// loading the full Quran corpus.

export const SURAHS = [
  { n: 1, name: 'Al-Fatiha', ayahs: 7 },
  { n: 2, name: 'Al-Baqarah', ayahs: 286 },
  { n: 3, name: 'Aal-Imran', ayahs: 200 },
...
const BY_NUM = new Map(SURAHS.map(s => [s.n, s]))
export function surahName(n) { return BY_NUM.get(n)?.name || `Surah ${n}` }
export function surahAyahs(n) { return BY_NUM.get(n)?.ayahs || 0 }
